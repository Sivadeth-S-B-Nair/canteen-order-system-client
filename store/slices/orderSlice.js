...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    updateOrderInList: (state, action) => {
      ...
      if (updated.status === "Picked Up" || updated.status === "CANCELLED") {
    ...
    cancelOrderInList: (state, action) => {
      const { orderId, refundRequest } = action.payload;
      const myIdx = state.myOrders.findIndex((o) => o.id === orderId);
      if (myIdx !== -1) {
